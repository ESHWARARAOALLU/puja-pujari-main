...
client = MongoClient('mongodb+srv://pujapujari:pujapujari@cluster0.adyp4.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0')
db = client['puja'] 
users= db["users"]
...
def main():
    ...
    return render_template('main.html',l=l,l1=l1,l2=l2,pList=pList,n=n,date1=date1)
    ...
    if user:
        if user.get('email') == email:
            if user.get('psw') == pwd:
                ...
                return render_template('main.html', user=user, msg1=msg1, l=l, l1=l1, pList=pList, n=n,date1=date1)
            else:
...
def addservice(id):
    ...
    if request.method == 'POST':
        ...
        # Splitting location string and handling ValueError
        try:
            name, loc = location.split('-')
        except ValueError:
            # If location doesn't contain the expected format
            # Redirect or render an error page as per your application's design
            return render_template('404.html', message='Invalid location format')
...
def viewImage(filename):
    ...
    return redirect(url_for('admin'))  # Redirect to admin or appropriate route
...
def addprasadam(id):
    if request.method == 'POST':
        ...
        # Check if qty is empty or not a number
        if not qty or not qty.isdigit():
            return render_template('404.html', message='Invalid quantity')
    ...
    return redirect(url_for('templepuj'))
